    ...
    public async Task<CheckoutResponseDto> PlaceOrderAsync(int idUsuario, CheckoutRequestDto request)
    {
        ...
        try
        {
            ...
            foreach (var item in carrito.Detalles)
            {
                ...
                {
                    inventarioSede.cantidad_disponible -= item.cantidad;
                }
            }
            ...
            var dirObj = new DireccionUsuario
            {
                ...
                direccion = request.DireccionLinea1 + ", " + request.Ciudad + ", " + request.CodigoPostal + ", Colombia",
                referencia = "Registrado desde Checkout"
            };
