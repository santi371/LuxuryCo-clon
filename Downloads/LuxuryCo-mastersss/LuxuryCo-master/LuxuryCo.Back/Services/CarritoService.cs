...
public class CarritoService : ICarritoService
{
    ...
    public async Task<bool> AddToCartAsync(int idUsuario, int idProducto, int cantidad, string? talla = null, string? color = null)
    {
        ...
        // Buscamos si ya existe el MISMO producto con la MISMA talla y color
        var detalleExistente = carrito.Detalles.FirstOrDefault(d => 
        ...
        // Validación CRÍTICA contra Inventario (simplificada al total por ahora, pero idealmente sería por variante)
        if (nuevaCantidad > producto.stock)
        {
            throw new Exception($"Stock insuficiente. Hay {producto.stock} en inventario total y estás intentando añadir {nuevaCantidad}.");
        }
    ...
    public async Task<bool> UpdateQuantityAsync(int idUsuario, int idProducto, int cantidad, string? talla = null, string? color = null)
    {
        var carrito = await InternalGetOrCreateCartAsync(idUsuario);
        var detalle = carrito.Detalles.FirstOrDefault(d => d.id_producto == idProducto && d.talla == talla && d.color == color);

        if (detalle == null) 
        {
            if (cantidad > 0) return await AddToCartAsync(idUsuario, idProducto, cantidad, talla, color);
            return false;
        }
