...
public class CartController : Controller
{
    ...
    private readonly string _apiBaseUrl = "http://localhost:5040/api/carrito";
    ...
    public async Task<IActionResult> Add(int productId, int quantity = 1, string? size = null, string? color = null)
    {
        ...
        try
        {
            var requestBody = new { IdProducto = productId, Cantidad = quantity, Talla = size, Color = color };
    ...
    [HttpPost]
    public async Task<IActionResult> Update(int id, int quantity, string? size = null, string? color = null)
    {
        AddAuthorizationHeader();
        try
        {
            var requestBody = new { Cantidad = quantity, Talla = size, Color = color };
            var json = JsonSerializer.Serialize(requestBody);
            var content = new StringContent(json, Encoding.UTF8, "application/json");
